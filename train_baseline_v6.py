"""
Baseline v6 — Two-stage transfer learning for overfitting control.
Stage A: frozen backbone, train head only (3 epochs).
Stage B: unfreeze all, differential LR with cosine annealing.
Class-agnostic regularization: dropout 0.5, WD 1e-3, label smoothing 0.1,
grad clipping 1.0, early stopping patience 3 on val macro-F1.
"""
...
SEED            = 42
DATA_DIR        = "./data"
RESULTS_DIR     = "./results/baseline_v6"
FIGURES_DIR     = "./figures/baseline_v6"
LOGS_DIR        = "./logs"
NUM_CLASSES     = 7
BATCH_SIZE      = 64
MAX_EPOCHS      = 30
...
with open(os.path.join(LOGS_DIR, "baseline_v6_history_s42.json"), "w") as f:
...
with open(os.path.join(RESULTS_DIR, "results.txt"), "w") as f:
    f.write(f"Baseline v6 — Two-stage Transfer Learning\n")
    f.write(f"{'='*50}\n")
...
plot_training_curves(epochs_all, tr_losses, vl_losses, "Loss",
                     "Baseline v6 — Loss (to best epoch)", "training_curves_loss.png",
                     best_epoch, cutoff=best_epoch)
plot_training_curves(epochs_all, tr_accs, vl_accs, "Accuracy (%)",
                     "Baseline v6 — Accuracy (to best epoch)", "training_curves_acc.png",
                     best_epoch, cutoff=best_epoch)
plot_training_curves(epochs_all, tr_f1s, vl_f1s, "Macro-F1",
                     "Baseline v6 — Macro-F1 (to best epoch)", "training_curves.png",
                     best_epoch, cutoff=best_epoch)

# 2. Full curves (appendix)
plot_training_curves(epochs_all, tr_losses, vl_losses, "Loss",
                     "Baseline v6 — Loss (full run)", "training_curves_full_loss.png",
                     best_epoch)
plot_training_curves(epochs_all, tr_accs, vl_accs, "Accuracy (%)",
                     "Baseline v6 — Accuracy (full run)", "training_curves_full_acc.png",
                     best_epoch)
plot_training_curves(epochs_all, tr_f1s, vl_f1s, "Macro-F1",
                     "Baseline v6 — Macro-F1 (full run)", "training_curves_full.png",
                     best_epoch)
...
ax.set_title("Baseline v6 — Test Confusion Matrix (Normalized)", fontsize=13, fontweight="bold")
...
ax.set_title("Baseline v6 — Per-class Test F1", fontsize=13, fontweight="bold")
ax.set_ylim(0, 1.05)
ax.legend()
...
ax.set_title("Baseline v6 — t-SNE of Test Features (512-d)", fontsize=13, fontweight="bold")
...
print(f"History          : {LOGS_DIR}/baseline_v6_history_s42.json")
