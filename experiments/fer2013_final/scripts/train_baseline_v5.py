"""Baseline v5: ResNet-18 with MILD augmentation (flip + crop) + regularization."""
import os
import json
import random
import time
...
def main():
    print("=" * 60)
    print("BASELINE v5: ResNet-18 — MILD augmentation (flip + crop)")
    print("  Dropout 0.4, weight decay 5e-4, label smoothing 0.1")
    ...
    # Mild augmentation: horizontal flip + random crop with 4px padding
    ...
    optimizer = torch.optim.Adam([
        {"params": backbone_params, "lr": 3e-5, "weight_decay": 5e-4},
        {"params": head_params, "lr": 1e-4, "weight_decay": 5e-4},
    ])
    ...
    with open(os.path.join(LOGS, f"baseline_v5_history_s{SEED}.json"), "w") as f:
    ...
    results = {
        ...
        "config": {
            "augmentation": "mild: RandomHorizontalFlip(0.5) + RandomCrop(48, padding=4)",
            "dropout": 0.4,
            "weight_decay": 5e-4,
            "label_smoothing": 0.1,
            "frozen_stages": "NONE — all layers unfrozen",
            "backbone_lr": 3e-5,
            "head_lr": 1e-4,
            "batch_size": 64,
            "max_epochs": 30,
    ...
    with open(os.path.join(RESULTS, f"baseline_v5_s{SEED}.json"), "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {os.path.join(RESULTS, f'baseline_v5_s{SEED}.json')}")
    ...
    print(f"  Disgust F1:       {test_per_f1[1]:.4f}")
    print(f"{'=' * 60}")
