"""Generate thesis figures for baseline v2 (300 dpi PNG)."""
import os
import json
import numpy as np
import matplotlib
...
FIGURES = os.path.join(BASE, "figures", "baseline_v2")
...
def fig2_training_curves():
    ...
    with open(os.path.join(LOGS, f"baseline_v2_history_s{SEED}.json")) as f:
        history = json.load(f)
    with open(os.path.join(RESULTS, f"baseline_v2_s{SEED}.json")) as f:
        results = json.load(f)
    ...
    fig.suptitle("Baseline v2 Training Curves (Augmented + Dropout 0.5)", fontsize=13, y=1.02)
    fig.tight_layout()
...
def fig3_confusion_matrix():
    ...
    fig.suptitle("Baseline v2 Confusion Matrix — Test Set", fontsize=13, y=1.02)
    fig.tight_layout()
...
def fig4_per_class_f1():
    ...
    ax.set_title("Per-Class F1 Score — Baseline v2 on Test Set")
    ax.set_ylim(0, 1.05)
...
def fig5_tsne():
    ...
    ax.set_title("t-SNE of Baseline v2 Test Features (512-d)")
...
def main():
    ...
    print("=" * 60)
    print("Generating baseline v2 figures")
    print("=" * 60)
