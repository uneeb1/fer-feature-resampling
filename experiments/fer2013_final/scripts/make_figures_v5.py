"""Generate thesis figures for baseline v5 (300 dpi PNG)."""
import os
import json
import numpy as np
import matplotlib
...
RESULTS = os.path.join(BASE, "results", "baseline_v5")
LOGS = os.path.join(BASE, "logs")
FIGURES = os.path.join(BASE, "figures", "baseline_v5")
...
def fig1_class_distribution():
    ...
    counts = [len(os.listdir(os.path.join(DATA, "train", cls))) for cls in CLASSES]
...
def fig2_training_curves():
    ...
    with open(os.path.join(LOGS, f"baseline_v5_history_s{SEED}.json")) as f:
        history = json.load(f)
    with open(os.path.join(RESULTS, f"baseline_v5_s{SEED}.json")) as f:
        results = json.load(f)
    ...
    ax1.plot(epochs, [h["train_accuracy"] for h in history], color="#2a78d6", linewidth=2, label="Train")
    ax1.plot(epochs, [h["val_accuracy"] for h in history], color="#eb6834", linewidth=2, label="Validation")
    ...
    ax2.plot(epochs, [h["train_loss"] for h in history], color="#2a78d6", linewidth=2, label="Train")
    ax2.plot(epochs, [h["val_loss"] for h in history], color="#eb6834", linewidth=2, label="Validation")
    ...
    fig.suptitle("Baseline v5 Training Curves (Mild Augmentation + Regularized)", fontsize=13, y=1.02)
    fig.tight_layout()
...
def fig3_confusion_matrix():
    ...
    fig.suptitle("Baseline v5 Confusion Matrix — Test Set", fontsize=13, y=1.02)
    fig.tight_layout()
...
def fig4_per_class_f1():
    ...
    ax.set_title("Per-Class F1 Score — Baseline v5 on Test Set")
    ax.set_ylim(0, 1.05)
...
def fig5_tsne():
    ...
    ax.set_title("t-SNE of Baseline v5 Test Features (512-d)")
...
def main():
    ...
    print("=" * 60)
    print("Generating baseline v5 figures")
    print("=" * 60)
