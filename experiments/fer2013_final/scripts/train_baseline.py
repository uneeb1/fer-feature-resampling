"""Step 1: Train baseline ResNet-18 on FER2013 (fully fine-tuned)."""
import os
import sys
import json
import random
...
def main():
    print("=" * 60)
    print("STEP 1: Training Baseline ResNet-18")
    print("=" * 60)
    ...
    with open(os.path.join(LOGS, f"baseline_history_s{SEED}.json"), "w") as f:
    ...
    if test_per_f1[1] == 0:
        print("\n*** WARNING: disgust F1 = 0 (possible finding, not a bug) ***")
    ...
    with open(os.path.join(RESULTS, f"baseline_s{SEED}.json"), "w") as f:
        json.dump(results, f, indent=2)
    print(f"\nResults saved to {os.path.join(RESULTS, f'baseline_s{SEED}.json')}")

    # Save test predictions for figure generation
    np.savez(os.path.join(RESULTS, f"test_predictions_s{SEED}.npz"),
             preds=np.array(test_preds), labels=np.array(test_labels))

    print("\n" + "=" * 60)
    print(f"STEP 1 COMPLETE — Best epoch: {best_epoch}, Val F1: {best_val_f1:.4f}, Test F1: {test_f1:.4f}")
    print("=" * 60)
